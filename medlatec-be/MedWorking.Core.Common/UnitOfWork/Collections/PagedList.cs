...
namespace MedWorking.Core.Common.UnitOfWork.Collections
{
    /// <summary>
    ...
    public class PagedList<T> : IPagedList<T>
    {
        ...
        internal PagedList(IEnumerable<T> source, int pageIndex, int pageSize, int rowModify)
        {
            if (source is IQueryable<T> querable)
            {
                ...
                Items = querable.Skip(((PageIndex - 1) * PageSize) + RowModify).Take(PageSize).ToList();
            }
            else
            {
                PageIndex = pageIndex;
                ...
                Items = source.Skip(((PageIndex - 1) * PageSize) + RowModify).Take(PageSize).ToList();
            }
        }
    ...
    internal class PagedList<TSource, TResult> : IPagedList<TResult>
    {
        ...
        public PagedList(IEnumerable<TSource> source, Func<IEnumerable<TSource>, IEnumerable<TResult>> converter, int pageIndex, int pageSize, int rowModify)
        {
            ...
            if (source is IQueryable<TSource> querable)
            {
                ...
                var items = querable.Skip(((PageIndex - 1) * PageSize) + RowModify).Take(PageSize).ToArray();
            ...
            else
            {
                ...
                var items = source.Skip(((PageIndex - 1) * PageSize) + RowModify).Take(PageSize).ToArray();
