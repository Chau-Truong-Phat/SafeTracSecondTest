...
using VNPT.ILIS.Core.Common.UnitOfWork.Collections;

namespace MedWorking.Core.Common.UnitOfWork;
...
public class Repository<TEntity> : IRepository<TEntity> where TEntity : class
{
    ...
    /// <returns></returns>
    public virtual IQueryable<TEntity> GetByWhere(Expression<Func<TEntity, bool>> predicate = null!,
    ...
    /// <summary>
    /// Lấy danh sách by where
    /// </summary>
    /// <param name="predicate"></param>
    /// <param name="orderBy"></param>
    /// <param name="disableTracking"></param>
    /// <returns></returns>
    public virtual IQueryable<TEntity> GetByWhereOrder(Expression<Func<TEntity, bool>> predicate = null!,
    ...
    public virtual async Task<IList<TEntity>> GetByTakeToListAsync(Expression<Func<TEntity, bool>> predicate = null!
        , Func<IQueryable<TEntity>, IOrderedQueryable<TEntity>> orderBy = null!,
        int take = 5, bool disableTracking = true)
    {
        IQueryable<TEntity> query = _dbSet;
        if (disableTracking)
        {
                                            ...
                                            bool disableTracking = true)
    {
        ...
        if (orderBy != null)
        {
            return orderBy(query).ToPagedList(pageIndex, pageSize, rowModify);
        }
        else
        {
        ...
        where TResult : class
    {
        ...
        if (orderBy != null)
        {
            return orderBy(query).Select(selector).ToPagedList(pageIndex, pageSize, rowModify);
        }
        else
        {
                                                       ...
                                                       CancellationToken cancellationToken = default(CancellationToken))
    {
        ...
        if (orderBy != null)
        {
            return orderBy(query).ToPagedListAsync(pageIndex, pageSize, rowModify, cancellationToken);
        }
        else
        {
            return query.ToPagedListAsync(pageIndex, pageSize, rowModify, cancellationToken);
        }
    }
        ...
        where TResult : class
    {
        ...
        if (orderBy != null)
        {
            return orderBy(query).Select(selector).ToPagedListAsync(pageIndex, pageSize, rowModify, cancellationToken);
        }
        else
        {
            return query.Select(selector).ToPagedListAsync(pageIndex, pageSize, rowModify, cancellationToken);
        }
    }

    public virtual TEntity FindOne(Expression<Func<TEntity, bool>> criteria = null!)
    {
        IQueryable<TEntity> query = _dbSet;
    ...
    /// </summary>
    /// <param name="entities">The entities.</param>
    public virtual void UpdateAsync(params TEntity[] entities) => _dbSet.UpdateRange(entities);
    ...
    public virtual void Delete(object id)
    {
        ...
        var key = _dbContext.Model.FindEntityType(typeInfo)!.FindPrimaryKey()!.Properties;
        var model = key[0];
        var property = typeInfo.GetProperty(model?.Name!);
        if (property != null)
        {
    ...
    /// </summary>
    /// <param name="predicate"></param>
    public virtual void DeleteByWhere(Expression<Func<TEntity, bool>> predicate = null!)
    {
        IEnumerable<TEntity> records = GetByWhere(predicate);
